...
int main()
{
	...
	auto path = std::format("{}\\config.ini",buf);
	/** 1. Load the INI file. */
	inicpp::IniManager _ini(fs::path(path).make_preferred().string());
	...
	std::string helloWorld_value = "ÄãºÃ£¬ÊÀ½ç"; // translate as : hello,world
	_ini.modify("other", "desc", helloWorld_value, "this test for std::wstring. comment it.");

	std::string wstr = _ini["other"].toString("desc");
	std::cout << "to wstring:\tother.desc= " << wstr << "\n\n";
